...
export function sceneQuestion(e: Env) {
  ...
  const eng = fade(t, 4.84, 6.4, 7.4, 9) * 0.08;
  if (eng > 0.002) {
    const R = Math.min(w, h) * 0.24;
    for (let i = 0; i < 22; i++)
      stroke(e, circlePts(cx, cy, R * (0.3 + i * 0.033), 90, 0.6, i), 1, 0.5, eng);
  }
  // Written quickly (5.04–6.84), then held 0.76 s before it breaks into circle A at 7.6.
  handwrite(e, Q1, seg(t, 5.04, 6.84), 1 - seg(t, 7.6, 8.4), cx, cy, q1Size(e), "center");
}
